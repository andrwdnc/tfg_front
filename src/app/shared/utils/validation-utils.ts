import { AbstractControl, ValidationErrors } from "@angular/forms";

export class ValidationUtils {
  public static isLatitude(control: AbstractControl): ValidationErrors | null {
    let latitude = control.value;

    if (isFinite(latitude) && Math.abs(latitude) <= 90) {
      return null;
    }

    return { isLatitude: true };
  }

  public static isLongitude(control: AbstractControl): ValidationErrors | null {
    let longitude = control.value;

    if (isFinite(longitude) && Math.abs(longitude) <= 180) {
      return null;
    }

    return { isLongitude: true };
  }

  public static MustMatch(controlName: string, matchingControlName: string) {
    return (group: AbstractControl) => {
        const control = group.get(controlName);
        const matchingControl = group.get(matchingControlName);

        if (!control || !matchingControl) {
            return null;
        }

        // return if another validator has already found an error on the matchingControl
        if (matchingControl.errors && !matchingControl.errors["mustMatch"]) {
            return null;
        }

        // set error on matchingControl if validation fails
        if (control.value !== matchingControl.value) {
            matchingControl.setErrors({ mustMatch: true });
        } else {
            matchingControl.setErrors(null);
        }
        return null;
    }
}

}
